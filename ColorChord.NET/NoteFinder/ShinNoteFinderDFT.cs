...
public static class ShinNoteFinderDFT
{
    ...
    Console.WriteLine("private static readonly Vector128<byte> SinWaveVecUpperB = Vector128.Create((byte)" + string.Join(", ", Values.Select(val => "0x" + (val >>   8).ToString("X2"))) + ");");
    Console.WriteLine("private static readonly Vector128<byte> SinWaveVecLowerB = Vector128.Create((byte)" + string.Join(", ", Values.Select(val => "0x" + (val & 0xFF).ToString("X2"))) + ");");
    */
    ...
    public static Vector256<short> GetSine256(Vector256<ushort> positions)
    {
        ...
        for (int i = 0; i < 16; i++) { Output[i] = GetSine(new() { NCLeft = positions[i], NCRight = 0 }, false).NCLeft; }
        return Vector256.LoadUnsafe(ref Output[0]);*/

        Vector128<byte> SinWaveVecUpperB = Vector128.Create((byte)0x00, 0x0C, 0x18, 0x23, 0x2D, 0x35, 0x3B, 0x3E, 0x3F, 0x3E, 0x3B, 0x35, 0x2D, 0x23, 0x18, 0x0C);
        Vector128<byte> SinWaveVecLowerB = Vector128.Create((byte)0x00, 0x7C, 0x7E, 0x8E, 0x41, 0x36, 0x20, 0xC4, 0xFF, 0xC4, 0x20, 0x36, 0x41, 0x8E, 0x7E, 0x7C);
        Vector256<ushort> TableBitmask = Vector256.Create((ushort)0b11111);

        Vector256<ushort> WholeLocations = Avx2.And(Avx2.ShiftRightLogical(positions, 11), TableBitmask); // Between 0 and 31
        Vector256<short> ValuesLeft;
        {
            Vector256<ushort> LocationModifiers = Avx2.ShiftRightArithmetic(Avx2.ShiftLeftLogical(WholeLocations, 11).AsInt16(), 15).AsUInt16();
            Vector128<byte> TableLocationsB = Sse2.PackUnsignedSaturate(WholeLocations.GetLower().AsInt16(), WholeLocations.GetUpper().AsInt16());
            Vector128<byte> ValuesLowerB = Ssse3.Shuffle(SinWaveVecLowerB, TableLocationsB);
            Vector128<byte> ValuesUpperB = Ssse3.Shuffle(SinWaveVecUpperB, TableLocationsB);
            Vector256<ushort> ValuesUnsigned = Vector256.Create(Sse2.UnpackLow(ValuesLowerB, ValuesUpperB), Sse2.UnpackHigh(ValuesLowerB, ValuesUpperB)).AsUInt16();
            ValuesLeft = Avx2.Subtract(Avx2.Xor(ValuesUnsigned, LocationModifiers).AsInt16(), LocationModifiers.AsInt16());
        }

        Vector256<ushort> AdjacentLocations = Avx2.And(Avx2.Add(WholeLocations, Vector256.Create((ushort)1)), TableBitmask); // Between 0 and 31
        Vector256<short> ValuesRight;
        {
            Vector256<ushort> LocationModifier = Avx2.ShiftRightArithmetic(Avx2.ShiftLeftLogical(AdjacentLocations, 11).AsInt16(), 15).AsUInt16();
            Vector128<byte> TableLocationsB = Sse2.PackUnsignedSaturate(AdjacentLocations.GetLower().AsInt16(), AdjacentLocations.GetUpper().AsInt16());
            Vector128<byte> ValuesLowerB = Ssse3.Shuffle(SinWaveVecLowerB, TableLocationsB);
            Vector128<byte> ValuesUpperB = Ssse3.Shuffle(SinWaveVecUpperB, TableLocationsB);
            Vector256<ushort> ValuesUnsigned = Vector256.Create(Sse2.UnpackLow(ValuesLowerB, ValuesUpperB), Sse2.UnpackHigh(ValuesLowerB, ValuesUpperB)).AsUInt16();
            ValuesRight = Avx2.Subtract(Avx2.Xor(ValuesUnsigned, LocationModifier).AsInt16(), LocationModifier.AsInt16());
        }

        Vector256<short> Values;
        {
            //Vector256<short> ValueDiffs = Avx2.ShiftLeftLogical(Avx2.Subtract(ValuesRight, ValuesLeft), 8); // TODO: This overflows!
            //Vector256<short> FractionalPositions = Avx2.And(Avx2.ShiftRightLogical(positions, 3 /* here */), Vector256.Create((ushort)0xFF)).AsInt16(); // Wasting the bottom 3 bits of position precision, but otherwise multiplication may overflow 32 bit lanes

            // Sneaky trick to avoid overflow: instead of doing ((ValueDiffs << 8) * (Positions >> 3)) >> 16, instead do ((ValueDiffs << 1) * ((Positions << 5) >> 1)) >> 16 which is the same (I think). Also doesn't waste the bottom bits of position precision :)
            Vector256<short> ValueDiffs = Avx2.ShiftLeftLogical(Avx2.Subtract(ValuesRight, ValuesLeft), 1); // TODO: Figure out if this is all correct
